...
mu_prem = Dense(LATENT_DIM)(prem)
mu_hypo = Dense(LATENT_DIM)(hypo)

sigma_prem = Dense(LATENT_DIM, activation='softplus')(prem)
sigma_hypo = Dense(LATENT_DIM, activation='softplus')(hypo)

kl_prem = kl_loss(mu_prem, sigma_prem)
kl_hypo = kl_loss(mu_hypo, sigma_hypo)

kl = Lambda(lambda x: x[0] + x[1], name='kl_sum')([kl_prem, kl_hypo])
x = CustomLossLayer()(kl)

z_prem = Lambda(lambda x: x[0] + x[1] * K.random_normal(shape=(LATENT_DIM,), mean=0., stddev=1.))([mu_prem, sigma_prem])
z_hypo = Lambda(lambda x: x[0] + x[1] * K.random_normal(shape=(LATENT_DIM,), mean=0., stddev=1.))([mu_hypo, sigma_hypo])

#--

joint = merge([z_prem, z_hypo], mode='concat')
